...
export default class PlayerElement extends HTMLElement {
    ...
    async connectedCallback() {
        if(DEBUG) console.log(`Connected player element: ${this.getAttribute("channel")} ${this.getAttribute("playlist")}`);
    ...
    checkIfPlayerIsActive() {
        if(document.activeElement.classList.contains("player-part") 
            || document.activeElement.localName === "player") {
       if(this.isTouchPointer) this.setIsActive();
        } else {
             if(this.isTouchPointer) this.setIsActive();
        }
...
customElements.define("art-player", PlayerElement);